...
public class VideosController {
    ...
    public ResEntity<VideoEntity> getVideo(@PathVariable String videoId) {
        ...
        return ResEntity.ok(videoEntity);
    }

    @ApiOperation(value = "获取多个视频", notes = "根据关键字获取多个视频")
    @GetMapping("/getVideos")
    public ResEntity<List<VideoEntity>> getVideos(VideoQueryModel model) {
        return ResEntity.ok();
    }

    @ApiOperation(value = "获取自己的视频", notes = "根据登录Header里面的Token获取自己的视频")
    @GetMapping("/getSelfVideos")
    public ResEntity<List<VideoEntity>> getSelfVideos() {
